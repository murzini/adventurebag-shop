...
export function DetailsPage({ item, config, onBack, onBuy, onCheckout }) {
  const handleCheckout = onBuy || onCheckout;
  if (!item) return null;

  const ctaCaption = String(config?.ctaCaption || "Add to cart");
  const ctaHelperText = String(
    config?.ctaHelperText || "Prototype checkout. No payment, no shipping, no account required."
  );
  const imageTipText = String(
          ...
          <div className="lg:col-span-7">
            <div className="relative grid grid-cols-[72px_1fr] gap-6">
              ...
              <div className="relative">
                ...
                {showGallery && (
                  <div className="mt-3 text-xs text-muted-foreground">{imageTipText}</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5">
            <div className="lg:sticky lg:top-24 flex max-w-lg flex-col gap-7">
              ...
              <div className="grid gap-4 rounded-3xl border bg-white p-6 shadow-sm">
                ...
                <div className="text-xs text-muted-foreground">{ctaHelperText}</div>
              </div>
            </div>
          </div>
        </div>
