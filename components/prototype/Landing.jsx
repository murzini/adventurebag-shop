...
export function Landing({ onStart, config }) {
  const heroTitle = config?.heroTitle || "Travel better with AdventureBag";
  const heroSubtitle =
    config?.heroSubtitle ||
  ...
  const heroCtaLabel = config?.heroCtaLabel || "Shop backpacks";

  const categories = Array.isArray(config?.categories) && config.categories.length
    ? config.categories
    : [
        { title: "Travel", description: "Explore backpacks built for travel.", cta: "Browse", tag: "travel" },
        { title: "Business", description: "Explore backpacks built for business.", cta: "Browse", tag: "business" },
        { title: "Hiking", description: "Explore backpacks built for hiking.", cta: "Browse", tag: "hiking" },
        { title: "Sports", description: "Explore backpacks built for sports.", cta: "Browse", tag: "sports" },
        { title: "Kids", description: "Explore backpacks built for kids.", cta: "Browse", tag: "kids" },
      ];

  const features = Array.isArray(config?.features) && config.features.length
    ? config.features
    : [
        { title: "Built to last", description: "Durable materials and reliable construction.", icon: "shield" },
        { title: "Fast delivery", description: "Quick, trackable shipping options.", icon: "truck" },
        { title: "Loved by owners", description: "Comfort-first designs used every day.", icon: "star" },
      ];

  const bottomTitle = config?.bottomTitle || "Explore our backpacks";
  const bottomSubtitle =
    config?.bottomSubtitle || "Browse all models and find the one that fits you best.";
  const bottomCtaLabel = config?.bottomCtaLabel || "View all backpacks";
      ...
      <motion.div {...fade} transition={{ duration: 0.25 }}>
        ...
        <section className="mt-14 sm:mt-24">
          ...
          <div className="mt-8 grid gap-4 sm:mt-10 sm:grid-cols-2 sm:gap-6 lg:grid-cols-5">
            {categories.map((item, idx) => (
              ...
              >
                <div className="text-base font-semibold sm:text-lg">{item?.title || "Category"}</div>
                <div className="mt-2 text-sm text-muted-foreground">
                  {item?.description || "Explore backpacks built for your needs."}
                </div>
        ...
        <section className="mt-14 grid gap-8 md:mt-24 md:grid-cols-3">
          {features.map((item, idx) => {
            ...
            return (
              <div className="flex gap-4" key={String(item?.title || idx)}>
                <div className="flex h-11 w-11 flex-none items-center justify-center rounded-2xl bg-[#3C5A7D] text-white">
                  <Icon className="h-5 w-5" />
                </div>
                <div>
                  <div className="text-base font-semibold">{item?.title || "Feature"}</div>
                  <div className="mt-1 text-sm text-muted-foreground">
                    {item?.description || "A short feature description."}
                  </div>
                </div>
              </div>
            );
        ...
        <section className="mt-16 text-center sm:mt-28">
          <h2 className="text-2xl font-semibold tracking-tight sm:text-3xl">{bottomTitle}</h2>
          <p className="mt-3 text-sm text-muted-foreground">{bottomSubtitle}</p>
          <div className="mt-8 flex justify-center sm:mt-10">
